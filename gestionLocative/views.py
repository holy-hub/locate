...
def payer_facture(request, location_id):
    ...
    if request.method == 'POST':
        ...
        if montant == location.propriete.montant:
            facture = Facture(utilisateur=request.user, propriete=location.propriete, montant=montant)
...
def accueil(request):
    propriete = Propriete.objects.filter(est_loue=False).order_by('nb_chambre')[:5].values()
    return render(request, 'locative/accueil.html', { 'title': 'accueil', 'propriete': propriete })

@login_required
def dashboard(request):
    documents = Document.objects.filter(utilisateur=request.user).order_by('titre')[:5].values()
    locations = Location.objects.filter(utilisateur=request.user).order_by('date_debut')[:5].values()
...
def create_propriete(request):
    if request.method == "POST":
        ...
        montant = request.POST["montant"]
        propriete = Propriete(nb_etage = nb_etage, adresse = adresse, ville = ville, designation = designation, montant = montant, code_postal = code_postal, description = description, nb_chambre = nb_chambre, surface = surface, proprietaire = request.user)
        propriete.save()
